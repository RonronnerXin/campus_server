        ...
        <div class="register-container">
            ...
            <form @submit.prevent="handleRegister" class="floating-form">
                ...
                <div class="input-group verification-group">
                    ...
                    <!-- <img class="img" referrerpolicy="no-referrer" @click="changeValiCode()" :src="img" /> -->
...
const sendVerificationCode=async ()=>{
    console.log("验证码", email.value)
    let res=await getEmailCode({email:email.value});
    if(res.code!=0){
        error(res.msg)
        return
    }
...
const sendVerificationCode = async () => {
  ...
  try {
    ...
    console.log("请求参数:", { email: email.value });
    let res = await getEmailCode({ email: email.value });
    ...
    console.error("请求异常:", err); // 捕获未处理的异常
    error("验证码发送失败，请检查网络");
  }
};
*/
const handleRegister = async () => {
    ...
    const res1 = await testEmailValidateInfo({
        validateCode: verificationCode.value,
        email: email.value
      });
    if (res1.message != '验证成功') {
        error(res1.message);
        return;
      }
    else if(res1.detail === '验证码错误或已过期'){
        error(res1.detail)
    }
